...
betaRegressionWeights <- function(formula, link = "probit", object, mc.cores, weights, ...){
  ...
  beta.regr <- function(object.part, formula, link, weights, ...){
    chr <- as.character(seqnames(rowRanges(object.part)))
    pred.meth.part <- methLevel(object.part)
    weights.part <- weights[rownames(pred.meth.part),]
    ...
    for(j in 1:nrow(pred.meth.part)){
      ...
      suppressWarnings(
                       lmodel <- try(summary(betareg(formula = as.formula(paste("pred.meth ~ ", formula[2])),
                                                     data=data, link = link, weights = weights.loop, ...)), silent=TRUE)
                       )
  ...
  summary.df.l <- mclapply(object.split, beta.regr, formula=formula, link=link, mc.cores=mc.cores, weights=weights, ...)
