...
FDR.cluster = 0.10)
#3046 CpG clusters rejected.

clusters.sig2 <- testClusters(locCor2,
FDR.cluster = 0.10)
#3210 CpG clusters rejected.


#trim the rejected CpG clusters to remove the not differentially methylated CpG sites
clusters.trimmed1 <- trimClusters(clusters.sig1, FDR.loc = 0.10)

clusters.trimmed2 <- trimClusters(clusters.sig2, FDR.loc = 0.10)
