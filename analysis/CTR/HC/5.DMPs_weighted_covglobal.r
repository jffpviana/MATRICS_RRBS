...
global_max <- max(coverage) #calculate the maximum coverage of all CpGs

weight_cov <- apply(coverage, 2, function(x) (x*10)/global_max)
...
write.csv(betaResults, file=paste0(output_dir, "DMPs_weighted_global_", cohort, "_", str_replace(region, "_ID", ""), ".csv")) #write out results
